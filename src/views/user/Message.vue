    ...
    <div class="message-container">
        ...
        <div class="item" v-for="(message,index) in messageList" :key="index">
            <div class="bell" :style="{backgroundColor: message.isRead ? 'rgb(246,246,246)' : 'rgb(239, 193, 87)'}">
                <i :style="{color: message.isRead ? 'rgb(51,51,51)' : 'rgb(246,246,246)'}" class="el-icon-message-solid">{{ message.isRead ? 'viewed' : 'new' }}</i>
            </div>
            <div>
    ...
    methods: {
        messageIsRead(){
            this.$axios.post(`/message/setRead`).then(res => {
                ...
                console.log("Set Viewed Error：", error);
            })
        },
        fetchMessage() {
